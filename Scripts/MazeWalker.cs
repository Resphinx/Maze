...
namespace Resphinx.Maze
{
    ...
    public class MazeWalker
    {
        ...
        public void Update()
        {
            ...
            Vector3 dir, fwd3d = new Vector3(camera.forward.x, 0, camera.forward.z).normalized;
            ...
            switch (Mazer.movementMode)
            {
                case MovementMode.Normal:
                    if ((foreMove = UserInputs.Hold(UserInputs.Forward)) || UserInputs.Hold(UserInputs.Back))
                    {
                        ...
                        feet = next;
                        camera.position = feet + (elevation + Mathf.Sin(currentBounceAngle) * 0.02f) * Vector3.up;
